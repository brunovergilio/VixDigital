...
uses
  ...
  FireDAC.Comp.Client, FireDAC.DBX.Migrate, ACBrUtil, MaskUtils;
...
type
  TFImportaExportaArquivo = class(TForm)
    ...
    Button6: TButton;
    QExtrato: TFDQuery;
    QExtratocodigo: TFDAutoIncField;
    QExtratodata: TDateTimeField;
    QExtratoorigem: TIntegerField;
    QExtratodestino: TStringField;
    QExtratofinalizado: TStringField;
    QExtratocampanha: TStringField;
    QExtratochat_bot: TStringField;
    QExtratoagentes: TStringField;
    QExtratotronco: TStringField;
    QExtratodid: TStringField;
    QExtratogrupo_atendimento: TStringField;
    QExtratoura: TStringField;
    QExtratovia: TStringField;
    QExtratoduracao_total: TStringField;
    QExtratoduracao_atendimento: TStringField;
    QExtratosipcode: TIntegerField;
    QExtratoabandono: TStringField;
    QExtratocpc: TIntegerField;
    QExtratoparam_categoria: TStringField;
    QExtratoparam_obs: TStringField;
    QExtratonome: TStringField;
    QExtratotelefone: TStringField;
    QExtratocpf: TStringField;
    QExtratodossie: TStringField;
    QExtratobanco: TStringField;
    QExtratoparam_ocorrencia: TStringField;
    QExtratodescricao: TStringField;
    QExtratodata_criacao: TDateTimeField;
    Edit1: TEdit;
    Label6: TLabel;
    QImportacao: TFDQuery;
    QExtratocod_importacao: TIntegerField;
    QExtratostatus: TStringField;
    QClienteNDG: TStringField;
    QClienteDOSSIE: TIntegerField;
    QClienteCOD_CARTEIRA: TIntegerField;
    QClienteNOME_LIDER: TStringField;
    QClienteNOME_OPERADOR: TStringField;
    QClienteETAPA: TStringField;
    QClienteEMPRESA_COBRADORA: TStringField;
    ...
    procedure cbxTipoCloseUp(Sender: TObject);
  private
...
begin
  if cbxTipo.ItemIndex = 2 then
    ...
    Button5.Click
  else if cbxTipo.ItemIndex = 7 then
    Button6.Click
  else
  begin
    if (cbxTipo.ItemIndex < 2) and (dblookupcombobox1.Text = '') and (cbxTipo.ItemIndex <> 7) then
    begin
...
//      cdscadas.Append;
//
//      cdscadas1.AsString  := Str_Lista[0]; //empresa_cobradora
//      cdscadas2.AsString  := Str_Lista[1]; //ndg
//      cdscadas3.AsString  := Str_Lista[2]; //dossie
//      cdscadas4.AsString  := Str_Lista[3]; //cpf/cnpj
//      cdscadas5.AsString  := Str_Lista[4]; //nome
//      cdscadas6.AsString  := Str_Lista[5]; //data_nascimento
//      cdscadas7.AsString  := Str_Lista[6]; //email
//      cdscadas8.AsString  := Str_Lista[7]; //logradouro
//      cdscadas9.AsString  := Str_Lista[8]; //numero
//      cdscadas10.AsString := Str_Lista[9]; //complemento
//      cdscadas11.AsString := Str_Lista[10]; //cep
//      cdscadas12.AsString := Str_Lista[11]; //bairro
//      cdscadas13.AsString := Str_Lista[12]; //cidade
//      cdscadas14.AsString := Str_Lista[13]; //estado
//      cdscadas15.AsString := Str_Lista[14]; //cod_carteira
//      cdscadas16.AsString := Str_Lista[15]; //nome_lider
//      cdscadas17.AsString := Str_Lista[16]; //nome_operador
//      cdscadas18.AsString := Str_Lista[17]; //tipo_pessoa
//      cdscadas19.AsString := Str_Lista[18]; //etapa
//      cdscadas20.AsString := Str_Lista[19]; //score
//
//      cdscadas.post;

      if (Length(SoNumero(Str_Lista[3])) = 14) or (Length(SoNumero(Str_Lista[3])) = 11) then
      begin
        with QCliente do
        begin
          Close;
          ...
          if Str_Lista[17] = '1' then
            ParamByName('CPFCNPJ').AsString := FormataCPF(SoNumero(Str_Lista[3]))
          else
            ParamByName('CPFCNPJ').AsString := FormataCNPJ(SoNumero(Str_Lista[3]));
          Open;
        end;
        ...
        begin
          ...
          QClienteNOME.AsString := Str_Lista[4];
          if Str_Lista[17] = '1' then
          begin
            QClienteTIPOPESSOA.AsString := 'F';
            QClienteCPF_CNPJ.AsString := FormataCPF(SoNumero(Str_Lista[3]));
          end
          else
          begin
             QClienteTIPOPESSOA.AsString := 'J';
             QClienteCPF_CNPJ.AsString := FormataCNPJ(SoNumero(Str_Lista[3]));
          end;
        end
        else
          QCliente.Edit;
        QClienteDTA_NASCIMENTO.AsDateTime := StrToDate(Str_Lista[5]);
        QClienteENDERECO.AsString := Str_Lista[7];
        QClienteNUMERO.AsString := Str_Lista[8];
        QClienteCEP.AsString := Str_Lista[10];
        QClienteSETOR.AsString := Str_Lista[11];
        QClienteCIDADE.AsString := Str_Lista[12];
        QClienteUF.AsString := Str_Lista[13];
        QClienteDTA_CAD.AsDateTime := Now;

        QClienteDOSSIE.AsString := Str_Lista[2];
        QClienteNDG.AsString := Str_Lista[2];
        QClienteE_MAIL.AsString := Str_Lista[6];
        QClienteCOD_CARTEIRA.AsString := Str_Lista[14];
        QClienteNOME_LIDER.AsString := Str_Lista[15];

        QClienteNOME_OPERADOR.AsString := Str_Lista[16];
        QClienteETAPA.AsString := Str_Lista[18];
        QClienteSCORE.AsString := Str_Lista[19];
        QClienteEMPRESA_COBRADORA.AsString := Str_Lista[1];
        QCliente.Post;
      end;
    end;
...
//    cdscadas.delete;
    ...
    showmessage('Ocorreu um erro na importação. Ln465');
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if Length(SoNumero(StringGrid1.Cells[0,I])) = 14 then
      begin
         QClienteCPF_CNPJ.AsString := Poem_Zeros(FormataCPF(SoNumero(StringGrid1.Cells[0,I])),18);
         QClienteTIPOPESSOA.AsString := 'J';
      end
      else
      begin
         //QClienteCPF_CNPJ.AsString := FormataCPF(StringGrid1.Cells[0,I]);


         QClienteCPF_CNPJ.AsString := Poem_Zeros(FormataCPF(SoNumero(StringGrid1.Cells[0,I])),14);
         QClienteTIPOPESSOA.AsString := 'F';
      end;
    end
    else
...
procedure TFImportaExportaArquivo.Button6Click(Sender: TObject);
var
  I, seq_cliente_codigo, codigo_cliente, seq_operacoes_codigo, seq_PARCELAS: Integer;
  var_noperacao, quantidade_registros, var_credor, var_cliente, remessa, vCpfCNPJ: string;
...
begin
  ...
  begin
    ...
    if (Length(SoNumero(StringGrid1.Cells[21,I])) = 14) or (Length(SoNumero(StringGrid1.Cells[21,I])) = 11) then
    begin
      QExtrato.Append;
      QExtratocod_importacao.AsInteger     := QImportacao.Fields[0].AsInteger + 1;
      QExtratodescricao.AsString           := Edit1.Text;
      QExtratodata.AsString                := StringGrid1.Cells[0,I];
      QExtratoorigem.AsString              := StringGrid1.Cells[1,I];
      QExtratodestino.AsString             := StringGrid1.Cells[2,I];
      QExtratofinalizado.AsString          := StringGrid1.Cells[3,I];
      QExtratocampanha.AsString            := StringGrid1.Cells[4,I];
      QExtratochat_bot.AsString            := StringGrid1.Cells[5,I];
      QExtratoagentes.AsString             := StringGrid1.Cells[6,I];
      QExtratotronco.AsString              := StringGrid1.Cells[7,I];
      QExtratodid.AsString                 := StringGrid1.Cells[8,I];
      QExtratogrupo_atendimento.AsString   := StringGrid1.Cells[9,I];
      QExtratoura.AsString                 := StringGrid1.Cells[10,I];
      QExtratovia.AsString                 := StringGrid1.Cells[11,I];
      QExtratoduracao_total.AsString       := StringGrid1.Cells[12,I];
      QExtratoduracao_atendimento.AsString := StringGrid1.Cells[13,I];
      QExtratosipcode.AsString             := StringGrid1.Cells[14,I];
      QExtratoabandono.AsString            := StringGrid1.Cells[15,I];
      QExtratocpc.AsString                 := StringGrid1.Cells[16,I];
      QExtratoparam_categoria.AsString     := StringGrid1.Cells[17,I];
      QExtratoparam_obs.AsString           := StringGrid1.Cells[18,I];
      QExtratonome.AsString                := StringGrid1.Cells[19,I];
      QExtratotelefone.AsString            := StringGrid1.Cells[20,I];
      if Length(SoNumero(StringGrid1.Cells[21,I])) = 14 then
         QExtratocpf.AsString := FormataCNPJ(SoNumero(StringGrid1.Cells[21,I]))
      else
         QExtratocpf.AsString := FormataCPF(SoNumero(StringGrid1.Cells[21,I]));
      QExtratodossie.AsString              := StringGrid1.Cells[22,I];
      QExtratobanco.AsString               := StringGrid1.Cells[23,I];
      QExtratoparam_ocorrencia.AsString    := StringGrid1.Cells[24,I];
      QExtratodata_criacao.AsDateTime      := Now;
      QExtrato.Post;
    end
    else
    begin
      Memo3.Lines.Add('Cadastro não realizado: CPF ou CNPJ incompleto: ' + StringGrid1.Cells[19,I] + ' - ' + StringGrid1.Cells[21,I]);
    end;
  end;
  frmdatamodule.uniconnection1.Commit;
  ShowMessage('Processo finalizado.');
...
procedure TFImportaExportaArquivo.cbxTipoCloseUp(Sender: TObject);
begin
  if cbxTipo.ItemIndex = 7 then
  begin
